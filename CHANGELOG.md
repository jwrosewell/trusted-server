...
- Added the pieces the Axum adapter needs to run as an appliance rather than a development server. It terminates TLS itself when `TRUSTED_SERVER_TLS_CERTIFICATE_PATH` and `TRUSTED_SERVER_TLS_PRIVATE_KEY_PATH` are both set, resolves its bind host through `EDGEZERO__ADAPTER__HOST`, opens a durable redb key-value store and an Edge Cookie identity store and answers every route with the startup error when either cannot be opened, generates and finalizes the Edge Cookie on the publisher path, caps the upstream response body at the same 10 MiB the other adapters use, reads the client address a trusted proxy forwarded when `TRUSTED_SERVER_TRUSTED_PROXY_HOPS` says how many proxies stand in front, sets `X-Forwarded-For` to the trusted client address on proxied requests, and serves `proxy.asset_routes`. Third-party URLs nested inside proxied assets can be rewritten to first-party paths derived from those routes, off by default, and URLs inside `noscript` and hostnames sharing the origin's prefix are now rewritten correctly. A `Dockerfile` and `docker-compose.yml` build and run the appliance, with the identity store on a volume.
- Added the 51Degrees module, `crates/geo/51degrees`, one vendor crate supplying a location provider, a device provider and an Edge Cookie identity provider from a single call to a 51Degrees cloud service. It follows the provider configuration convention, running when `[integration] provider` names `fiftyone_degrees`, reading its settings from `[integration.fiftyone_degrees]` and refusing a setting it does not know, and each of its three providers serves a request only where `[geo]`, `[device]` or `[ec]` selects the module. A 51Did posted by the browser or carried in a cookie is verified through the 51Degrees SDK against the signing key in force when it was created before it is trusted, and the browser module delegates the high entropy client hints to the configured endpoint. The Axum adapter offers the module to a deployment, and offering it is not enabling it. See the [51Degrees integration guide](docs/guide/integrations/51degrees.md).
