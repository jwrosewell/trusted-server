...
///
/// Offering a module is not enabling it. A builder here runs only when
/// `[integration] provider` names its id, so a configuration that names no
/// vendor behaves exactly as it did before the crate was linked in.
...
mod tests {
    ...
    fn state_with_uninjected_provider() -> AppState {
        ...
        AppState {
            ...
            // It drives the per-request path too, which builds its services
