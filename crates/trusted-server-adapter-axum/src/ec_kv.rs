...
impl EcKvStore for AxumEcKvStore {
    ...
    fn key_exists(&self, key: &str) -> Result<bool, Report<TrustedServerError>> {
        // The same transaction and table a lookup reads, so the answer is as
        // consistent as the lookup itself, and an expired row counts as
        // absent for the same reason it does there.
        let read = self
            .database
            .begin_read()
            .change_context(Self::error("begin a read"))?;
        let table = read
            .open_table(EC_TABLE)
            .change_context(Self::error("open the table"))?;
        let row = table.get(key).change_context(Self::error("read a key"))?;
        Ok(row.is_some_and(|row| !Self::is_expired(row.value().3)))
    }
