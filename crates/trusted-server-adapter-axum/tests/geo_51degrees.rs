...
/// Builds the registry the adapter builds at startup, from the compiled
/// auction plan and this crate's builder.
fn registry_with(settings: &Settings) -> IntegrationRegistry {
    let plan = Arc::new(compile_auction_plan(settings).expect("should compile the auction plan"));
    IntegrationRegistry::with_plan_and_registrations(settings, plan, &[geo_51degrees::builder()])
        .expect("should build the registry from this crate's builder")
}
...
fn the_selector_resolves_this_vendors_provider_through_the_adapter() {
    ...
    let router = TrustedServerApp::routes_with_registrations(settings, &[geo_51degrees::builder()]);
...
/// A deployment naming this module for identity starts up.
///
/// Offering the builder is not enough on its own. The start-up check resolves
/// the Edge Cookie selection separately, and it has to be handed the provider
/// the registry resolved, because the request path is handed exactly that.
/// Until it was, a deployment writing `[ec] provider = "fiftyone_degrees"` was
/// refused at start-up by a binary that contained the module and whose request
/// path would have used it, so the identity half of this vendor could not be
/// switched on at all.
///
/// This drives the same entry point a deployment does, rather than resolving
/// the provider from the registry as the tests above do, because those two ask
/// different questions and only this one fails when the check and the request
/// path disagree.
#[test]
fn a_deployment_naming_this_module_for_identity_starts_up() {
    ...
    assert!(
        router.is_ok(),
        "the start-up check must accept a selection the request path resolves, or the          identity provider is unreachable in every real deployment"
    );
}
...
fn a_51degrees_identifier_survives_core_read_back_verbatim() {
    ...
    let registry = registry_with(&settings);
