...
/// and it names the `[integration.fiftyone_degrees]` configuration block they
/// share, which is read when `[integration] provider` names the module.
...
pub fn register(
    ...
    Ok(Some(
        builder
            .with_geo_provider(Arc::new(FiftyOneDegreesGeo::new(Arc::clone(&client))))
...
/// `[integration] provider` names this module.
///
/// # Errors
///
/// Returns an error when the configuration block cannot be parsed or fails
...
mod tests {
    ...
    fn no_configuration_block_declares_nothing() {
        ...
        assert!(
            ...
            "should report the module as not selected"
        );
    }
