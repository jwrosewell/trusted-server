...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| KV store                                   | Persistent `redb` file at `.edgezero/trusted_server_kv.redb`, locked for exclusive use. Startup fails rather than silently dropping identity and consent state                                    |
| Geo lookup                                 | None of its own, so with no `[geo] provider` selected a request resolves the policy's declared jurisdiction. A provider from an integration module can be selected                               |
